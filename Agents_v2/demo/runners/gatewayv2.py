...
async def main(args):
    gateway_agent = await create_agent_with_args(args, ident="consortium")
    ...
    try:
        ...
        agent = GatewayAgent(
            ...
            anoncreds_legacy_revocation=gateway_agent.anoncreds_legacy_revocation,
        )
        ...
        options += "    (X) Exit?\n[1/2/3/4/{}{}T/X] ".format(
            "5/6/7/8/" if gateway_agent.revocation else "",
        ...
        async for option in prompt_loop(options):
            ...
            if option is None or option in "xX":
                ...
                if gateway_agent.aip == 10:
                    offer_request = gateway_agent.agent.generate_credential_offer(
                ...
                try:
                    ...
                    pass
            elif option == "7" and gateway_agent.revocation:
                try:
                    resp = await gateway_agent.agent.admin_POST(
                        f"/revocation/active-registry/{gateway_agent.cred_def_id}/rotate",
                        {},
                    )
                    gateway_agent.agent.log(
                    ...
                    pass
            elif option == "8" and gateway_agent.revocation:
                states = [
                    "init",
                    "generated",
                    "posted",
                ...
                if state not in states:
                    state = "active"
                try:
                    resp = await gateway_agent.agent.admin_GET(
