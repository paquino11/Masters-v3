...
class AriesAgent(DemoAgent):
    ...
    async def handle_present_proof_v2_0(self, message):
        ...
        if state == "request-received":
            ...
            if(self.prefix == "GATEWAY" and proof["verified"] == "true"):
                print("TRUEEEE")
                uuid_obj = uuid.uuid4()
                userUUID = str(uuid_obj)
                await self.admin_POST(
                    f"/connections/{self.connection_id}/send-message",
                    {"content": userUUID},
                )
                self.claim_set.add(userUUID)
    ...
    async def handle_basicmessages(self, message):
        ...
        print(message)
        #if(message["content"] == "Start Streaming data from device")
...
class AgentContainer:
    ...
    async def create_schema_and_cred_def_ownership(
        self,
        schema_name: str,
        schema_attrs: list,
        version: str = None,
        ...
        if self.cred_type == CRED_FORMAT_INDY:
            # need to redister schema and cred def on the ledger
            self.cred_def_id_ownership = await self.agent.create_schema_and_cred_def(
                schema_name, schema_attrs, self.revocation, version=version
            )
            return self.cred_def_id_ownership
        elif self.cred_type == CRED_FORMAT_JSON_LD:
            # TODO no schema/cred def required
            pass
            return None
