...
async def main(args):
    ...
    def save_OEM_agent_fabric():
        url = "http://192.168.182.138:3000/regariesagent"
        #payload = {"string": ipfs_hash}
        response = requests.post(url)
        
        if response.status_code == 200:
            print("Agent successfully to Fabric!")
        else:
            print("Failed to save agent to the server.")
